...
public class MinioProvider : IFilesProvider
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var putObjectArgs = new PutObjectArgs()
            .WithBucket(fileDataDto.InfoMy.BucketName)
            .WithStreamData(fileDataDto.Stream)
            .WithObjectSize(fileDataDto.Stream.Length)
            .WithObject(fileDataDto.InfoMy.FilePath.FullPath);
        try
        {
            await _minioClient.PutObjectAsync(putObjectArgs, cancellationToken);
            return fileDataDto.InfoMy.FilePath;
        }
        catch (Exception e)
        {
            _logger.LogError(
                e, "Fail to upload file in minio with path {path} with bucket {backet} ",
                fileDataDto.InfoMy.FilePath.FullPath,
                fileDataDto.InfoMy.BucketName);
       ...
       // return Errors.General.NotFound();
        ...
        try
        {
            await IfBucketsNotExistCreateBucket(
                filesData.Select(x => x.InfoMy.BucketName)
                , cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var args = new RemoveObjectArgs()
                .WithBucket(deleteDataFile.Bucket)
                .WithObject(deleteDataFile.PetId.ToString());

            await _minioClient.RemoveObjectAsync(args, cancellationToken);

            _logger.LogInformation($"REMOVE object in minio: {deleteDataFile.PetId.ToString()}");
            return $"REMOVE object in minio: {deleteDataFile.PetId.ToString()}";
        }
        catch (Exception e)
        {
